...
def test_agent_team_skills_config_group_registered():
    groups = {group["key"]: group for group in AGENT_TEAM_CONFIG_GROUPS}

    assert "skills" in groups
    assert groups["skills"]["title_key"] == "agent_team.config_group_skills"
    assert groups["skills"]["keys"] == [
