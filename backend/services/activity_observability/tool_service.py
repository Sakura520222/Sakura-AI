...
class ToolService:
    ...
    def _message_manifest_ids(
        ...
        try:
            ...
            raise ValueError("context revision has an invalid message manifest") from exc
        if (
    ...
    async def create_tool_execution(
        ...
        *,
        ...
        async for db in self._session_scope():
            async with db.begin():
                work_unit, _, _ = await self._get_parent_chain(db, work_unit_id, thread_id)
                ...
                if existing is not None:
                    expected = arguments_json if sensitivity in {SENSITIVITY_PUBLIC, SENSITIVITY_INTERNAL} else None
                    if existing.name != name or existing.arguments_sensitivity != sensitivity or existing.arguments_json != expected:
                        raise ConflictError("tool call id already has different payload")
                    return existing
                stored_arguments = (
                    arguments_json
                    if sensitivity in {SENSITIVITY_PUBLIC, SENSITIVITY_INTERNAL}
                    else None
                )
        ...
        async for db in self._session_scope():
            async with db.begin():
                ...
                if execution.status in TERMINAL_TOOL_STATUSES:
                    same = execution.status == status and execution.result_sensitivity == result_sensitivity
                    same = same and (
                        execution.result_json == result_json
                        if result_sensitivity in {SENSITIVITY_PUBLIC, SENSITIVITY_INTERNAL}
                        else execution.result_hash == self._digest(result_json)
                    )
                    if not same:
                        raise ConflictError("terminal tool execution conflicts with prior result")
                    return
                if execution.status != TOOL_STATUS_RUNNING:
    ...
    async def get_tool_execution(self, tool_execution_id: int) -> ActivityToolExecution | None:
        async for db in self._session_scope():
    ...
    async def append_assistant_message(
        ...
        *,
        ...
        async for db in self._session_scope():
            async with db.begin():
                work_unit, thread, _ = await self._get_parent_chain(db, work_unit_id, thread_id)
                if thread is None:
                ...
                locked_thread = await db.get(ActivityThread, thread.id, with_for_update=True)
        ...
        async for db in self._session_scope():
            async with db.begin():
                ...
                payload_json = json.dumps(
                    safe, ensure_ascii=False, default=str, sort_keys=True, separators=(",", ":")
                )
                    ...
                    for tc in tool_calls:
                        ...
                        db.add(
                            ActivityToolExecution(
                                ...
                                name=parsed["name"],
                                arguments_json=parsed["arguments"] or None,
    ...
    async def mark_tool_execution_running(self, work_unit_id: int, tool_call_id: str) -> None:
        async for db in self._session_scope():
            async with db.begin():
                execution = (
                    await db.execute(
                ...
                invocation.current_phase = work_unit.current_phase
                session = await db.get(ActivityObservabilitySession, work_unit.session_id)
                if session is not None:
                    session.last_active_at = self._clock()
                await append_lifecycle_event(
                    db,
    ...
    async def capture_reasoning_artifact(
        ...
        *,
        ...
        key = build_compatibility_key(provider_family, protocol_family, model_family, endpoint_scope)
        persist = policy.should_persist_payload(availability, provider_family, protocol_family)
        if policy.capture_mode == "artifact" and persist and self._encryption_provider is None:
            raise RuntimeError("artifact capture requires an encryption provider")
        encrypted = self._encryption_provider.encrypt(payload) if persist and payload is not None and policy.capture_mode == "artifact" else None
        safe_summary = payload if persist and policy.capture_mode == "safe_summary" else None
        retention = self._clock() + timedelta(days=policy.retention_days) if policy.retention_days is not None else None
        async for db in self._session_scope():
            async with db.begin():
                ...
                artifact = ActivityNativeArtifact(
                    ...
                    visibility=policy.artifact_visibility if policy.capture_mode == "artifact" else policy.summary_visibility,
                    ...
                    payload_safe_summary=safe_summary,
                    payload_hash=self._digest(payload),
                    retention_expires_at=retention,
                    replay_allowed=policy.capture_mode == "artifact" and availability in {REASONING_ENCRYPTED_OPAQUE, REASONING_PROVIDER_EXPOSED},
                )
                db.add(artifact)
                await db.flush()
                return artifact
    ...
    async def _audit(self, db, artifact_id: int | None, actor: str, scope: str | None, outcome: str) -> None:
        db.add(
            ActivityArtifactAccessLog(
                artifact_id=artifact_id,
        ...
        async for db in self._session_scope():
            async with db.begin():
                ...
                if artifact is None:
                    ...
                    return None
                attempt = await db.get(ActivityModelAttempt, artifact.attempt_id)
                work_unit = await db.get(ActivityInvocationWorkUnit, attempt.work_unit_id if attempt else -1)
                invocation = await db.get(ActivityInvocation, work_unit.invocation_id if work_unit else -1)
                session = await db.get(ActivityObservabilitySession, invocation.session_id if invocation else -1)
                identity = await db.get(ActivityResourceIdentity, session.resource_identity_id if session else -1)
                if self._artifact_authorizer is None or any(item is None for item in (attempt, work_unit, invocation, session, identity)):
                ...
                decision = decision if isinstance(decision, ArtifactAuthorization) else ArtifactAuthorization(bool(decision))
                if not decision.allowed:
                    await self._audit(db, artifact.id, actor, decision.authorization_scope, "denied")
                    return None
                await self._audit(db, artifact.id, actor, decision.authorization_scope, "allowed")
                return AuthorizedArtifactView(
                    ...
                    payload_safe_summary=artifact.payload_safe_summary if decision.can_display and artifact.availability != REASONING_ENCRYPTED_OPAQUE else None,
    ...
    @staticmethod
    def can_replay_artifact(artifact: ActivityNativeArtifact, *, compatibility_key: str, policy_allows: bool) -> bool:
        return bool(artifact.replay_allowed and policy_allows and artifact.compatibility_key == compatibility_key)
