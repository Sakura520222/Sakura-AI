...
"""
    ...
    def _format_comment(
        ...
        # 添加标题
        strategy_info = get_strategy_config().get_strategy(strategy)
        strategy_name = strategy_info.get(
            "name", "Code Review" if is_en else "代码审查"
        )
        report_title = (
            "🌸 Sakura AI Review Report" if is_en else "🌸 Sakura AI 审查报告"
        )
        ...
        footer = (
            "This comment was auto-generated by Sakura AI Reviewer"
            if is_en
            else "此评论由 Sakura AI Reviewer 自动生成"
        )
        lines.append("\n---\n")
