...
*⏳ Sakura is thinking... This may take from a few seconds to minutes depending on code complexity.*
"""
            else:
...
Please check system logs or contact the administrator.
...
"""
    ...
    def _format_comment(
        ...
        # 添加标题
        strategy_info = get_strategy_config().get_strategy(strategy)
        strategy_name = strategy_info.get("name", "Code Review" if is_en else "代码审查")
        report_title = "🌸 Sakura AI Review Report" if is_en else "🌸 Sakura AI 审查报告"
        lines.append(f"# {report_title} - {strategy_name}\n")

        # 添加评分（如果有）
        overall_score = review_result.get("overall_score")
        if overall_score:
            score_label = "Overall Score" if is_en else "整体评分"
            lines.append(f"## 📊 {score_label}: {overall_score}/10\n")

        # 添加 AI 生成的完整审查内容
        summary = review_result.get("summary", "")
        if summary:
            view_detail = "View detailed review report" if is_en else "查看详细审查报告"
            lines.append(f"<details><summary>📋 {view_detail}</summary>\n\n")
        ...
        # 添加页脚
        footer = "This comment was auto-generated by Sakura AI Reviewer" if is_en else "此评论由 Sakura AI Reviewer 自动生成"
        lines.append("\n---\n")
