...
from backend.core.ai_providers import (  # noqa: E402
    list_builtin_providers,
    list_provider_catalog,
)
from backend.core.ai_protocol import account_store  # noqa: E402
from backend.core.ai_protocol.account_probe import probe_account  # noqa: E402
from backend.core.ai_protocol.endpoint_security import validate_provider_base_url  # noqa: E402
...
async def list_ai_accounts(user: dict = Depends(require_api_super_admin)):
    ...
    return success_response(
        data={"accounts": [a.to_public_dict() for a in accounts]}
    )
...
async def save_ai_account(
    ...
    if provider_id not in {provider.id for provider in list_builtin_providers()}:
        return error_response(f"未知的 AI 厂商 / unknown AI provider: {body.provider_id}")
    ...
    logger.info(f"AI 账号已保存 / account saved: {saved.id} ({saved.name}), by={user['sub']}")
