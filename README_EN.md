...
- **AI Model**: Save multiple OpenAI, Anthropic, Gemini, DeepSeek, Qwen, GLM, MiniMax, Kimi, Grok, Mistral, aggregator, local-model, or custom-compatible accounts in WebUI "AI Config", and configure separate account/fallback chains for main, summary, and Agent Team roles. Discovered model lists are persisted and selectable through model tags. Per-model advanced configuration independently sets context window, max output, image input, thinking mode/effort, and temperature/top_p/top_k capabilities/defaults. Built-in remote accounts accept only official HTTPS endpoints; `custom` / `custom-anthropic` can use public HTTPS endpoints or HTTP/HTTPS localhost and private-network compatible endpoints. Legacy single-model fields in "General Config" remain as migration fallback
- **Auxiliary Model**: Set the summary role in WebUI "AI Config" to either follow the main model or use a separate account for summarization, label recommendation, context compression, and other lightweight tasks
