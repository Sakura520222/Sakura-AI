...
- **Webhook 处理器行为一致性**：处理同一事件类型的多个 webhook handler（如 Stripe, Paddle 等），其前置检查、核心逻辑、日志输出和错误处理模式应保持一致，不一致处需明确理由或标 minor/major。
- **幂等性审查要点**：审查幂等性改进时，不仅要看核心函数是否幂等，还要同步检查所有调用方是否适配了新的返回值语义，以及是否因新的幂等性而优化控制流和事务处理。
- **相似代码块对比审查**：当变更涉及多个结构相似的模块时，应系统性对比它们在相同上下文下的实现，任何不一致都可能是缺陷或技术债。
- **用户可见错误处理评估**：面向终端用户的 UI 路由，错误处理策略变更（如静默容错变为暴露 500）是显著体验退化，除非有明确降级页面或监控需求，否则标 major。
...
- **状态机变更审查**：涉及 worker/task/status/webhook 的 PR，应要求审查新增状态、允许的前置/后置状态、重复/乱序事件行为；对未显式处理的状态路径，要求注释或测试。
- **Webhook 事件幂等检查**：webhook 处理变更必须检查签名验证覆盖新增分支、重复投递幂等、延迟事件通过 sha/timestamp/status 防护、缺字段 payload 安全失败。
- **数据库模型变更全链路审查**：新增列/索引/唯一约束必须检查迁移文件、nullable/default、历史数据回填、约束冲突、索引命名跨库兼容，以及应用是否依赖迁移而非自动建表。
- **并发安全审查**：审查涉及状态机转换、共享内存缓存或"读取-检查-执行"操作的变更时，必须进行并发场景分析，说明操作的原子性或幂等性保证。
- **模块级缓存治理**：模块级 dict 缓存必须有清理策略、容量上限说明或使用弱引用；审查时需拷问容量、清理时机、进程生命周期内是否会成为负担。
- **权限过滤中禁止 truthiness 判断**：对 user/tenant/org/repo 等 scope 字段，必须用 `if scope is not None` 而非 `if scope`，避免空字符串、0、空集合绕过过滤。
- **timezone-aware datetime 迁移**：`datetime.utcnow()` 替换为 `datetime.now(timezone.utc)` 时，需确认下游是否与 naive datetime 比较、JSON 序列化、模板展示兼容。
- **状态匹配键唯一性**：`repo+branch` 常不足以定位任务；优先要求 pr_number、head_sha、status 或 active 唯一约束，并在代码中说明业务保证。
- **Worker 资源清理**：_cancel_events、_background_tasks、pending prompts 等资源必须在终态、异常、提前 return 时清理；新增 early return 必须检查资源释放。
- **Fake/mock 与生产查询一致性**：fake session 的过滤、排序、limit、异常行为必须模拟生产查询；复杂查询宜补 sqlite/集成测试，否则测试可能比生产"更正确"。
- **事件驱动闭环审查**：webhook+worker+DB/Agent Team 类变更重点查状态进入/退出、重复/乱序/延迟事件、PR closed/synchronized/dismissed、并发更新、唯一约束冲突捕获和终态资源清理。
...
- **增量审查完整性验证**：增量审查必须区分"当前增量提交通过"与"整个 PR 可合并"；历史阻断问题状态必须逐项列明，避免"完整性幻觉"。
- **Release PR 描述与 diff 强制核对**：Release/changelog 型 PR 中，若描述列出功能集合但当前增量 diff 未体现，应至少标记为 suggestion/minor，要求确认描述来源。
- **历史问题修复验证责任**：标记历史问题"已修复"时，必须基于当前 diff 的具体代码位置确认，给出行号和模式；未验证的只能说"本轮未覆盖"。
- **结构化评论质量收紧**：只有具体、可定位、可执行的问题才能生成 review comment；审查摘要、评分、历史状态表、"无问题"声明不得进入 comment 列表。
...
- **"用户可见"与"系统内部"的错误处理分层**：面向用户的接口错误处理应以用户体验和恢复为先；内部服务可更倾向暴露错误以便快速发现；审查时需对两类场景保持敏感。
