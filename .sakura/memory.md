...
## Gitflow 同步冲突与 Issue 自动化（#581-#585 高频主题）

- **自动同步勿直接 merge**：main→develop 先 `git merge --no-commit --no-ff` 预检；有冲突则经临时分支建 draft PR 而非直接合并/建 Issue，并附 `git diff --name-only` 冲突文件清单；受保护分支走 PR 审查勿直接 push
- **优先级看影响面**：冲突阻断 develop CI（阻塞全部在途 PR）宜升 high；已自愈仅一次告警可 low，但须复核自动化健壮性
- **重复检测防误报**：标题相似≠重复；须比对分支方向、冲突 SHA、时间点；同类不同次标 related/similar；关键字组合+近 30 天时间窗过滤防漏报
...
- **.sakura/ 生成文件冲突**：宜重跑生成脚本而非手工编辑；设合并窗口降冲突频率；解决前先备份（临时分支/tar 导出，diff -r 校验防丢反思记录）
- **估算与标题**：工作量给区间+最坏上限（15-45min），Issue 模板加"预计耗时"字段；原标题已清晰（分支+方向）时勿强行改写
