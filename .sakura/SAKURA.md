...
## 11. 最新反思沉淀（PR #395–#410 Check Run 同步、增量审查隧道视野与 Issue 分析质量）
- PR #395 incr2 评论同步重构：字符串匹配关联删除是高危反模式；解耦操作须验证下游消费方适配（`issues` 字典行为变更）；"正确语义≠安全变更"——修正历史错误行为须验证下游隐式依赖；净减代码须区分"删除逻辑"还是"删除债务"。
- PR #396 审查协议容错增强：AI 输出容错须补负面测试；Score clamping 是行为契约变更须验证边界；移除截断/限制等安全网须提供显式替代；Diff 解析功能须覆盖空/二进制/重命名/大文件；外部内容嵌入 GitHub markdown 须评估 ``` 注入；跨模块直通数据流是管道架构脆弱点。
- PR #397 Issue 分析协议迁移：对等迁移须双向行为验证（字段集、验证规则、降级条件逐项对比），禁止仅凭"风格一致"正面评价；防御性 `.get()` 掩盖字段缺失须标 minor；固定字符串 untrusted data 边界须验证注入逃逸；信封协议变更应有标准检查清单。
- PR #397 incr2 返回结构枚举化：早返回路径的字段集一致性须主动对比，不能依赖下游 `.get()` 容错；返回结构内部分类字段新增取值须列全已有取值并建议提取为常量或 Enum；增量审查应以新代码为线索重新审视旧代码完整性。
- PR #398 GitHub Checks 集成 Issue：集成类 Issue 降级策略是必问项；异步架构的状态可视化面临状态同步延迟；优先级应基于用户信任影响判定；状态枚举与第三方 API 模型对齐是非常规映射须指出。
- PR #399–#400 增量审查队列机制（核心架构变更）：数据源替换须三层检查——结构兼容性、消费方适配、行为规则迁移；"会话恢复"模式须验证 system prompt/tool 定义/运行时参数是否被当前配置覆盖；状态机中间状态无逃脱路径 = major；新增核心服务文件（>200 行）须独立审查单元；大面积新增测试文件须抽样审查异常分支。
- PR #402 Webhook/队列集成：≥3 个≤10 行小变更文件须逐文件说明变更意图，删除行必须追溯；核心链路静默失败至少 major；新增 DB 表须对照查询验证索引覆盖；低成本→高成本模型切换须论证必要性；PR 描述与 diff 量级差异应标 major；宽泛异常兜底日志须含异常类型或 `exc_info`。
- PR #403 提示词契约变更：提示词即接口契约，修改约束须追踪下游解析代码验证隐式假设耦合；"纯文本变更"≠零风险变更——被代码隐式依赖的文本格式约定修改与接口变更同级；全量重审应评估系统影响面而非仅文件数；approve 不等于沉默。
- PR #404 incr1-4 Check Run 缓存与增量审查隧道视野：实例缓存将无状态服务变为有状态服务是架构级变更（行数少≠风险低）；布尔标志参数（finalize）全路径传播验证是独立检查项；head_sha 在异步链路中漂移须验证初始化→传递→消费→异常→终态→反向影响全部路径；增量审查存在"隧道视野"陷阱——微观穷尽≠宏观完整，须设"抬头步骤"强制回看 PR 描述与历史问题；suggestion 初始偏低会产生锚定效应；满分须通过"历史 major 清零"硬性门槛。
- Issue #405 分支感知搜索：可行性分析遗漏阻断前提（AI 如何获知分支列表？）；回退默认分支在 AI 分析场景是"有害容错"；可行性粒度须均匀，参考模式须做行为对齐分析。
- Issue #406 互助点星功能提议：产品契合度应是第一道过滤，合规/ToS 风险应直接导向 wontfix 而非 low；`security` 标签仅用于当前已存在的漏洞，不用于假设性风险；功能提议三阶判断：定位过滤→合规过滤→优先级。
- Issue #407 WebUI 宽度约束：UI 行为异常应优先考虑 bug 分类；CSS 修复须区分 hidden/auto/scroll 交互语义；good first issue 置信度须反映隐性门槛。
- Issue #408 WorkspaceSecurityError：安全异常须区分"防线误伤"与"防线击穿"，标签和修复思路截然不同；平级目录架构下路径校验是高频陷阱；重复检测须附带触发场景比对。
- Issue #409 元分析格式崩溃：格式契约级联故障——单标签缺失致整条分析链路崩溃，各模块须独立解析、独立降级；回退默认值伪装成分析结果，须显式标记"分析失败"；内部错误信息不得泄漏到用户界面。
- Issue #410 元分析质量评估：自引用/递归类 Issue 须具备递归深度检测；边界场景置信度应主动衰减；评估类 Issue 须转化为可验证改进任务；"提示词即接口契约"须具象化为逐行追踪耦合点。
- 通用规则新增/强化：(1) 数据源-消费方联动删除须 grep 全仓逐项确认；(2) 替换式重构的半完成状态是高频 bug 根因；(3) 队列服务固定审查维度：入队幂等、消费并发、消息确认/重试、死信处理、积压监控；(4) 过滤操作须 debug 级输出被过滤元素标识；(5) 增量审查摘要须含"本轮增量范围说明"以消除决策完整性模糊性；(6) 删除文件须同时 grep README/CHANGELOG 等软引用。

*最后更新：基于 PR #395–#410 Check Run 同步、增量审查隧道视野与 Issue 分析质量审查经验，累计反思 455 次*