...
## 11. 最新反思沉淀（PR #387–#395 协议重构、安全修复与依赖图增强）
- PR #387 协议重构与常量统一：emoji 常量集中化须全仓 grep 验证旧模式清除；`scan_report_service.py` 多格式渲染路径是 DRY 盲点；配置降级静默回退必须有 `logger.warning`（可观测性硬规则）；审查评分须反映交付完成度，重构不彻底应扣分。
- PR #389 安全修复与工作区重构：`str(e)` 信息泄露修复须追问同类模式是否全仓存在，安全修复审查需"侦查"思维；工作区 per-repo 锁驱逐竞态需文档化失效模式及应对策略；`_refresh_ai_clients` 选择性调用须论证完整性（为何其他执行点不需要刷新）。
- PR #393 权限枚举扩展（`none`→`unknown`）：字符串枚举反模式——新增状态值须全仓 `grep` 级调用点扫描；PR 描述列表是"遗漏线索"而非"完成清单"，计数对比 diff；测试与代码同源偏差，新增测试数与描述点一一对应时警惕盲点；语义迁移 PR 执行"零遗漏"标准。
- PR #394 依赖图增量优化：第三方 API 状态值匹配须注释官方枚举来源；可选参数恒定回退值（恒为0）应标 major 而非 minor；测试参数须触发关键分支（数据量>阈值）；`_build_prompts` 签名变更虽内部方法仍需调用方扫描。
- PR #395 评论同步重构：字符串匹配关联删除是高危反模式（依赖 LLM 输出随机性）；解耦操作须验证下游消费方适配（`issues` 字典行为变更）；PR 内提交主题偏离须显式标注；净减代码重构须区分"删除逻辑"还是"删除债务"。
- 通用规则新增/强化：(1) 安全修复类 PR 须评估同类模式全仓清理；(2) 局部配置刷新须列所有消费点说明一致性保证；(3) 辅助函数迁移须逐项列出调用点迁移状态；(4) 技术债结论"可接受"须给后续行动建议；(5) 过滤操作须 debug 级输出被过滤元素标识；(6) "正确语义≠安全变更"——修正历史错误行为须验证下游隐式依赖。

*最后更新：基于 PR #387–#395 协议重构、安全修复与依赖图增强审查经验，累计反思 435 次*
