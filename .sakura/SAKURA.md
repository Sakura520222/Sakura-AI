...
## 11. 最新反思沉淀（PR #378–#386）
- PR 描述漂移仍是高频漏检：Release PR、功能增强 PR 均出现描述声称文档/迁移已更新但 diff 无证据，必须前置核对并标注。
- Dashboard/统计聚合接口属于敏感数据接口：必须显式接收 `scope_user` 权限参数，同类统计函数过滤口径必须对齐，禁止用 truthiness（`if val`）代替 `is not None` 做权限判断。
- timezone-aware datetime 迁移不是纯机械替换：`utcnow()` → `now(timezone.utc)` 后须检查下游是否与 naive datetime 混算、DB 字段、JSON 序列化、测试 mock 是否同步。
- Release PR 专项检查：版本号/changelog/migration/依赖锁文件/文档与实际 diff 必须一致，不能因为是聚合 PR 就降低审查标准。
- 数据库模型新增列/索引/约束/枚举必须检查迁移文件、nullable/default、历史数据回填、唯一约束对存量数据是否冲突，项目依赖自动建表时需说明机制。
- 结构化评论噪声控制持续关键：摘要/评分/正向评价/checklist/"无问题"声明禁止生成 review comment；同一根因只保留一条；严重程度全篇唯一且与最终决策一致（approve 时 major 须说明非阻断理由）。
- Webhook 处理器行为一致性：同一事件类型的多个 handler（Stripe/Paddle/Alipay/NOWPayments）前置检查、核心逻辑、错误处理模式必须一致，不一致处需明确理由。
- 用户可见错误处理策略变更：面向终端 UI 的路由将静默容错改为抛 500 是显著体验退化，需作者明确设计决策（保留容错 vs 暴露给监控），不得默认变更。
- 增量审查边界必须分层声明：明确列出"已验证修复"/"本轮未覆盖"/"新增风险"，approve 仅适用于当前增量提交，不代表整个 PR 所有历史问题已关闭。增量审查必须同时包含"双视角"：(1) 当前增量提交质量；(2) 对整个 PR 可合并状态的影响（尤其历史阻断问题是否仍存在）。
- 幂等性改进审查：不仅看核心函数是否幂等，必须同步检查所有调用方是否适配新返回值语义，以及是否应优化控制流和事务处理（如条件提交空操作）。
- 语义变更（删除过滤条件/改变查询行为）审查：必须评估对现有调用方影响、是否所有场景适用新语义、方法文档是否更新契约。
- 异步路径同步 IO 检查清单：async def 路由/函数调用链中须系统性检查文件操作、同步网络请求、同步 DB 操作、CPU 密集计算是否阻塞事件循环。修复同步 IO 时必须扫描同一模块中所有同类阻塞点一并修复。
- 相似服务结构对比审查：多个服务类（AIReviewer/IssueAnalyzer/EmbeddingService/RerankerService）实现相似功能（配置刷新、工具管理）时，必须对比实现一致性，差异需明确理由或标记技术债。
- 配置校验显式化：使用字符串匹配（如 endswith）进行安全校验处，必须使用枚举/白名单，或注释声明校验边界与信任前提。
- 测试覆盖缓存/副作用路径：引入缓存或状态变更的代码，测试必须覆盖缓存命中路径（验证未重建）和副作用清理路径（验证 close() 后状态清空）。
- PR 原子性检查：纯重构与业务逻辑变更混杂在同一 PR 时应标记为 minor，建议拆分。提交信息应准确反映变更性质（refactor vs fix vs test）。
- 机械性替换完整性验证：全局性模式替换（日志格式/API 迁移）审查时，要求作者提供全量扫描证据（grep/lint），或审查者抽样验证无遗漏。
- 共享逻辑测试覆盖一致性：多处理器/多 handler 共用同一函数时，测试不应仅覆盖单一路径，应检查参数化测试覆盖或说明测试策略。
- 条件分支事务行为测试：涉及 commit/rollback 条件调用的代码，测试应同时断言正向（commit 被调用）和反向（commit/rollback 未调用）行为。
- 异常处理可观测性底线：面向用户的接口层恢复 try/except 时，异常至少以 warning/error 级别记录，完全静默的错误处理禁止用于关键路径。
- Agent Team PR review 闭环专项：状态机矩阵、任务匹配键、TOCTOU 竞态、PR closed/review dismissed 后事件到达。
- Agent Shell 工具输出必须双层限额：执行阶段防内存膨胀，进入模型上下文前防 token/成本/注入；"移除截断"默认触发安全和资源审查。
- fake/mock 行为必须贴近生产查询：复杂 SQL fake 应补轻量集成测试或真实 sqlite session，不能比生产更严格或更宽松。
- 相似代码块（如多 webhook handler）审查应"找不同"：系统性对比相同上下文下的实现，任何不一致都可能是缺陷或技术债。
- 测试的语义验证：函数签名/行为契约变更时，即使测试通过，也应检查 mock 配置是否准确反映生产环境预期行为。
- 技术债显性化：发现大范围遗留问题（如42处残留）时，应量化技术债规模并建议创建后续 Issue 跟踪，使之成为可管理的项目任务。

*最后更新：基于 PR #378–#386 增量审查经验，累计反思 415 次*
