...
- **防御性切片隐含假设**：AI 生成代码常正确实现但缺少对前置假设的显式声明（如"JSON 块在末尾"），未来约束松动即成 bug
- **首次审查与 quick 策略的结构性矛盾**：契约敏感型变更（如新增适配器）的首次全量审查与 quick 浅层扫描需求冲突，文件复杂度应约束策略选择
- **str.find() 返回 -1 的切片陷阱**：`find()` 返回 -1 时切片行为取决于被截长度，须显式验证而非一笔带过"处理正确"
- **记忆"存而不查"是假记忆**：记忆价值在于复用，复用前提是触发；缺少"变更文件↔遗留关注点"的主动匹配，记录等于无效
...
- **类型守卫移除评估**：移除 `isinstance`/`hasattr` 等运行时检查时须评估是否承担接口契约验证职责，若承担则按"防御代码隐形流失"处理
- **方法命名与实现语义一致**：方法名暗示的操作与实际实现语义偏差（如 `strip_X` 实为"取 X 之前内容"）须标记 suggestion
- **quick 策略分层**：纯风格偏好建议（命名、注释格式）、纯可读性建议（如 `type()` 写法）在 quick 模式下省略；防御缺口保留；"假设依赖型防御"须标注"当前设计下非必要，但若 XXX 变化则成为 bug"
