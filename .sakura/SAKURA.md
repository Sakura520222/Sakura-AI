...
## 4. 硬规则（重点）
- **全绿Gate提供链接**：任意GitHub Actions失败须标记为error，PR合并前须提供全绿run链接
...
- **安全脱敏分级**：except块对外响应必须标注错误类别(#USER_INPUT_ERROR/#INTERNAL_ERROR)
...
- **外部CLI版本声明**：workflow调用外部二进制须明确最低版本并验证
...
- 负向路径contract测试强制：新增错误分支须提供fail‑closed场景测试

## 6. 知识库维护
- .sakura/更新须增量追加或精确修改，禁止覆盖式重写
- 元数据修改须提供计算依据；核心文件修改视为与代码变更同等重要
- 单一真值源：URL/版本等常量须从中心位置引用，避免硬编码
- 文档-代码同步：关键行为必须在代码中实现

## 7. 最新反思要点（累计20次）
- **PR539**：外部CLI版本声明、负向路径contract测试、全库消费点搜索、错误信息结构化、CI环境兼容性
- **ISSUE541**：分类准确性、标签完整性与置信度、重复检测复核、标题改写规范、需求拆分建议
- **ISSUE538**：优先级业务影响评估、标签细化（vision/multimodal）、token预算统一、测试覆盖、死配置闭环
- **PR533(incr6)**：文档-代码核验（以文验码）、证据链刚性约束、真值单向追溯、微小变更深度阅读
- **PR533(incr5)**：全绿Gate提供链接强制、增量审查触发全库搜索、Docker参数统一生成、测试覆盖双向断言、跨层错误码同步、文档-代码同步检查
- **历史关键**：配置清理验证、循环约束验证、守护进程幂等性、镜像版本绑定、全库引用搜索、环形依赖检测、安全异常分类、统一取消机制、状态枚举同步、跨平台抽象、POSIX mock对齐
...
*最后更新：基于 PR #539 / ISSUE #541/538 与 PR #533 (incr5/6) 反思，累计反思 20 次*