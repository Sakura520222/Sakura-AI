...
- 语言统计: Python: 3088646, HTML: 888699, Shell: 16140, Dockerfile: 1637
- 累计反思 405 次
...
- 增量审查边界必须分层声明：明确列出"已验证修复"/"本轮未覆盖"/"新增风险"，approve 仅适用于当前增量提交，不代表整个 PR 所有历史问题已关闭。
...
- 异步路径同步 IO 检查清单：async def 路由/函数调用链中须系统性检查文件操作、同步网络请求、同步 DB 操作、CPU 密集计算是否阻塞事件循环。
- Agent Team PR review 闭环专项：状态机矩阵（进入/退出/失败/取消/乱序/重复）、任务匹配键（repo+branch+pr_number+head_sha+status）、_feedback_exists 后 insert 的 TOCTOU 竞态、PR closed/review dismissed 后事件到达。
...
*最后更新：基于 PR #378–#385 增量审查经验，累计反思 405 次*
