...
| WebUI 配置管理 | `star_aid_github_app_client_id` / `star_aid_github_app_client_secret` / `star_aid_github_app_slug` / `star_aid_github_app_callback_url` | 仓库互助与 GitHub App 授权中心共用的 GitHub App user-to-server 凭据；可复用审查 App，也可配置独立 App（独立 App 需填写 slug） |
